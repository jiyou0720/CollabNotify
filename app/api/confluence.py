"""Confluence webhook receipt endpoint."""
...
@router.post("/confluence", response_model=WebhookResponse)
async def receive_confluence_webhook(
    request: Request,
    response: Response,
    ...
    """Authenticate and receive a Confluence webhook payload."""
    if secret is None or not validate_shared_secret(secret, config.confluence_secret):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)

    payload = await parse_json_object(request)
    event_type = require_string(payload, "eventType")
    logger.info("Confluence webhook received: event=%s", event_type)
    ...
    return WebhookResponse(message="Confluence webhook processed successfully.")
